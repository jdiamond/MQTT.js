...
function MqttClient (streamBuilder, options) {
  ...
  this.on('connect', function (connack) {
    this.connected = true;

    function resubscribeToTopics (next) {
      var topics = Object.keys(that.subscriptions);

      if (topics.length) {
        if (connack.sessionPresent) {
          topics = topics.filter(function (topic) {
            return 0 === that.subscriptions[topic];
          });
          that.subscribe(topics, next);
        } else {
          that.subscribe(that.subscriptions, next);
        }
      } else {
        next();
      }
    ...
    function resendInflightPackets () {
    ...
    resubscribeToTopics(resendInflightPackets);
  });
...
MqttClient.prototype.subscribe = function () {
  ...
  this.outgoing[packet.messageId] = function (err, granted) {
    if (!err) {
      Object.keys(granted).forEach(function (qos, i) {
        self.subscriptions[subs[i].topic] = +qos;
      });
    }
